...
def test_omni_car_grid_command_features_are_sensor_derived() -> None:
    ...
    env._obstacle_xy[0] = np.asarray([[0.70, 0.0], [10.0, 10.0], [10.0, 10.0]], dtype=np.float32)
...
def test_omni_car_auto_zero_command_snaps_without_smoothing_tail() -> None:
    ...
    env.init_state()
    env._commands[:] = np.asarray([[1.0, -0.4, 0.3], [0.8, 0.1, -0.5]], dtype=env._dtype)
    env._raw_commands[:] = np.asarray([[0.0, 0.0, 0.0], [0.05, 0.02, 0.01]], dtype=env._dtype)
...
def test_omni_car_human_command_overrides_selected_agent() -> None:
    ...
    np.testing.assert_allclose(env._raw_commands[env._human_command_env_ids], expected_command_rows)
    np.testing.assert_allclose(env._commands[env._human_command_env_ids], expected_command_rows)
    ...
    expected_next_command_rows = np.broadcast_to(next_command, (env._human_command_env_ids.size, 3))
...
def test_omni_car_front_obstacle_rewards_stop_over_forward_push() -> None:
    ...
    target_push_clearance_motion = float(env._reward_components["clearance_target_motion"][0])
...
def test_omni_car_target_collision_penalizes_predicted_penetration() -> None:
    env = registry.make(
        "OmniCarGridAvoidance",
        sim_backend="mujoco",
        num_envs=1,
    ...
    stop_reward = env._compute_reward(
        ...
        policy_action=np.zeros((1, 3), dtype=np.float32),
        prev_clearance=prev_clearance,
        prev_pose=prev_pose,
    )
    stop_target_collision = float(env._reward_components["target_collision"][0])
    push_reward = env._compute_reward(
        np.zeros((1, 3), dtype=np.float32),
        policy_action=np.asarray([[1.5, 0.0, 0.0]], dtype=np.float32),
        prev_clearance=prev_clearance,
        prev_pose=prev_pose,
    )
    push_target_collision = float(env._reward_components["target_collision"][0])

    assert prev_clearance[0] > 0.0
    assert stop_target_collision == pytest.approx(0.0)
    assert push_target_collision < 0.0
...
def test_omni_car_target_collision_ignores_parallel_side_wall_motion() -> None:
    ...
    env.init_state()
    env._commands[:] = np.asarray([[1.0, 0.0, 0.0]], dtype=np.float32)
    env._obstacle_xy[0, 0] = np.asarray([0.0, -0.50], dtype=np.float32)
    env._obstacle_radius[0, 0] = 0.20
    env._nearest_clearance[:] = env._compute_clearance(np.asarray([0], dtype=np.int32))
    prev_pose = env._pose.copy()
    prev_clearance = env._nearest_clearance.copy()
    ...
    parallel_target_collision = float(env._reward_components["target_collision"][0])
    env._compute_reward(
        np.zeros((1, 3), dtype=np.float32),
        policy_action=np.asarray([[0.0, -1.0, 0.0]], dtype=np.float32),
        prev_clearance=prev_clearance,
        prev_pose=prev_pose,
    )
    closing_target_collision = float(env._reward_components["target_collision"][0])

    assert 0.0 < prev_clearance[0] < env._cfg.reward.target_collision_margin_m
    assert parallel_target_collision == pytest.approx(0.0)
    assert closing_target_collision < 0.0
    env.close()


def test_omni_car_blocked_projection_penalizes_only_blocked_forward_push() -> None:
    env = registry.make(
        "OmniCarGridAvoidance",
        sim_backend="mujoco",
        num_envs=1,
    ...
    env.init_state()
    env._commands[:] = np.asarray([[1.0, 0.0, 0.0]], dtype=np.float32)
    env._obstacle_xy[0] = np.asarray([[0.55, 0.0], [10.0, 10.0], [10.0, 10.0]], dtype=np.float32)
...
def test_omni_car_clearance_opening_rewards_moving_away_from_near_obstacle() -> None:
    env = registry.make(
        "OmniCarGridAvoidance",
        sim_backend="mujoco",
        num_envs=1,
    ...
    toward_opening = float(env._reward_components["clearance_opening"][0])

    env._commands[:] = np.zeros((1, 3), dtype=np.float32)
    env._pose[:] = env._predict_pose_from_action(prev_pose, away_action)
    env._nearest_clearance[:] = env._compute_clearance(np.asarray([0], dtype=np.int32))
    idle_reward = env._compute_reward(
        away_action,
        prev_clearance=prev_clearance,
        prev_pose=prev_pose,
    )

    assert away_opening > 0.0
...
def test_omni_car_clearance_opening_requires_projection_when_path_is_free() -> None:
    env = registry.make(
        "OmniCarGridAvoidance",
        sim_backend="mujoco",
        num_envs=1,
    ...
    env.init_state()
    env._commands[:] = np.asarray([[1.0, 0.0, 0.0]], dtype=np.float32)
    env._obstacle_xy[0, 0] = np.asarray([0.20, -0.42], dtype=np.float32)
    env._obstacle_radius[0, 0] = 0.22
    env._nearest_clearance[:] = env._compute_clearance(np.asarray([0], dtype=np.int32))
    prev_pose = env._pose.copy()
    prev_clearance = env._nearest_clearance.copy()

    lateral_only = np.asarray([[0.0, 0.4, 0.0]], dtype=np.float32)
    env._pose[:] = env._predict_pose_from_action(prev_pose, lateral_only)
    env._nearest_clearance[:] = env._compute_clearance(np.asarray([0], dtype=np.int32))
    env._compute_reward(
        lateral_only,
        prev_clearance=prev_clearance,
    ...
    lateral_opening = float(env._reward_components["clearance_opening"][0])

    projected_escape = np.asarray([[0.2, 0.4, 0.0]], dtype=np.float32)
    env._pose[:] = env._predict_pose_from_action(prev_pose, projected_escape)
    env._nearest_clearance[:] = env._compute_clearance(np.asarray([0], dtype=np.int32))
    env._compute_reward(
        projected_escape,
        prev_clearance=prev_clearance,
...
def test_omni_car_clearance_target_opening_rewards_projected_side_escape() -> None:
    env = registry.make(
        "OmniCarGridAvoidance",
        sim_backend="mujoco",
        num_envs=1,
    ...
    outward_reward = env._compute_reward(
        np.zeros((1, 3), dtype=np.float32),
        policy_action=np.asarray([[0.2, 0.4, 0.0]], dtype=np.float32),
        prev_clearance=prev_clearance,
        prev_pose=prev_pose,
    )
    outward_opening = float(env._reward_components["clearance_target_opening"][0])
    inward_reward = env._compute_reward(
        np.zeros((1, 3), dtype=np.float32),
        policy_action=np.asarray([[0.2, -0.2, 0.0]], dtype=np.float32),
        prev_clearance=prev_clearance,
        prev_pose=prev_pose,
    )
    inward_opening = float(env._reward_components["clearance_target_opening"][0])
    lateral_only_reward = env._compute_reward(
        np.zeros((1, 3), dtype=np.float32),
        policy_action=np.asarray([[0.0, 0.4, 0.0]], dtype=np.float32),
        prev_clearance=prev_clearance,
        prev_pose=prev_pose,
    )
    lateral_only_opening = float(env._reward_components["clearance_target_opening"][0])

    assert outward_opening > 0.0
    assert inward_opening == pytest.approx(0.0)
    assert lateral_only_opening == pytest.approx(0.0)
    assert outward_reward[0] > inward_reward[0]
    assert lateral_only_reward[0] == pytest.approx(0.0)
    env.close()
...
def test_omni_car_blocked_lateral_escape_rewards_biased_side_escape() -> None:
    env = registry.make(
        "OmniCarGridAvoidance",
        sim_backend="mujoco",
        num_envs=1,
    ...
    env.init_state()
    env._commands[:] = np.asarray([[1.0, 0.0, 0.0]], dtype=np.float32)
    env._obstacle_xy[0, 0] = np.asarray([0.52, -0.24], dtype=np.float32)
    env._obstacle_radius[0, 0] = 0.22
    env._nearest_clearance[:] = env._compute_clearance(np.asarray([0], dtype=np.int32))
    prev_pose = env._pose.copy()
    prev_clearance = env._nearest_clearance.copy()
    ...
    away_action = np.asarray([[0.0, 1.0, 0.0]], dtype=np.float32)
    env._pose[:] = env._predict_pose_from_action(prev_pose, away_action)
    env._nearest_clearance[:] = env._compute_clearance(np.asarray([0], dtype=np.int32))
    away_reward = env._compute_reward(
        away_action,
    ...
    away_escape = float(env._reward_components["blocked_lateral_escape"][0])

    assert stop_escape == pytest.approx(0.0)
    assert away_escape > 1.0
    assert away_reward[0] > stop_reward[0]
    env.close()


def test_omni_car_lateral_drift_penalizes_command_line_deviation() -> None:
    env = registry.make(
        "OmniCarGridAvoidance",
        sim_backend="mujoco",
        num_envs=1,
        env_cfg_override={
            ...
            "reward": {
                ...
                "clearance_target_motion": 0.0,
                "clearance_opening": 0.0,
                "clearance_target_opening": 0.0,
                "blocked_lateral_escape": 0.0,
                "lateral_drift": 10.0,
                "lateral_drift_margin_m": 0.05,
                "lateral_drift_scale_m": 0.20,
                "target_collision": 0.0,
                "blocked_projection": 0.0,
                "blocked_speed": 0.0,
                "blocked_stop": 0.0,
    ...
    env._commands[:] = np.asarray([[1.0, 0.0, 0.0]], dtype=np.float32)
    env._raw_commands[:] = env._commands
    env._command_anchor_pose[:] = env._pose
    env._obstacle_xy[0, 0] = np.asarray([10.0, 10.0], dtype=np.float32)
    env._nearest_clearance[:] = env._compute_clearance(np.asarray([0], dtype=np.int32))

    centered_reward = env._compute_reward(np.zeros((1, 3), dtype=np.float32))
    centered_drift = float(env._reward_components["lateral_drift"][0])

    env._pose[0, 1] = 0.30
    drifted_reward = env._compute_reward(np.zeros((1, 3), dtype=np.float32))
    drifted_drift = float(env._reward_components["lateral_drift"][0])

    assert centered_drift == pytest.approx(0.0)
    assert drifted_drift < 0.0
    assert drifted_reward[0] < centered_reward[0]
    env.close()
...
def test_omni_car_zero_command_penalizes_raw_policy_target() -> None:
    env = registry.make(
        "OmniCarGridAvoidance",
        sim_backend="mujoco",
        num_envs=1,
    ...
    env.init_state()
    env._commands[:] = 0.0
    executed = np.asarray([[0.0, 0.0, 0.0]], dtype=np.float32)

    idle_reward = env._compute_reward(executed)
    target_reward = env._compute_reward(
        executed,
        policy_action=np.asarray([[0.4, -0.2, 0.3]], dtype=np.float32),
    )
...
def test_omni_car_cnn_model_forward_actor_and_critic() -> None:
    cfg = OmniCarGridAvoidanceCfg()
    obs_dim = cfg.grid.size * cfg.grid.size + 3 + 3 + 3 + cfg.obs_history_len * 9 + 2
...
def test_omni_car_cnn_gru_model_accepts_configurable_cnn_channels() -> None:
    cfg = OmniCarGridAvoidanceCfg()
    actor_obs_dim = (
        cfg.grid_history_len * cfg.grid.size * cfg.grid.size
        + 3
    ...
    assert actor_out.shape == (2, 3)
    assert actor.grid_encoder[0].out_channels == 6
    assert actor.grid_encoder[2].in_channels == 6
    assert actor.grid_encoder[2].out_channels == 10
    assert actor.grid_encoder[4].in_channels == 10
    assert actor.grid_encoder[4].out_channels == 14
    assert actor.grid_encoder[7].in_features == 14 * 10 * 10


def test_omni_car_cnn_gru_model_rejects_invalid_cnn_channels() -> None:
    cfg = OmniCarGridAvoidanceCfg()
    actor_obs_dim = (
        cfg.grid_history_len * cfg.grid.size * cfg.grid.size
        + 3
...
def test_omni_car_cnn_gru_command_skip_bounds_residual_action() -> None:
    cfg = OmniCarGridAvoidanceCfg()
    actor_obs_dim = (
        cfg.grid_history_len * cfg.grid.size * cfg.grid.size
        + 3
    ...
    actor_obs = torch.zeros((2, actor_obs_dim), dtype=torch.float32)
    commands = torch.tensor([[0.5, -0.25, 0.4], [-0.2, 0.1, -0.3]], dtype=torch.float32)
    command_start = cfg.grid_history_len * cfg.grid.size * cfg.grid.size
    actor_obs[:, command_start : command_start + 3] = commands
    actor = OmniCarGridCNNGRUModel(
        TensorDict({"actor": actor_obs}, batch_size=2),
        ...
        gru_hidden_dim=8,
        command_skip_scale=1.0,
        residual_action_scale=10.0,
        residual_action_mode="tanh",
        residual_action_limit=[0.1, 0.2, 0.3],
        distribution_cfg={
            "class_name": "rsl_rl.modules.distribution.GaussianDistribution",
            "init_std": 0.5,
            "std_type": "scalar",
